...
function(kiriki_configure_bgfx target_name)
    ...
    find_package(unofficial-bgfx CONFIG QUIET)
    if(TARGET unofficial::bgfx::bgfx)
        target_compile_definitions(${target_name} PRIVATE KIRIKIRI_HAS_BGFX=1)
        target_link_libraries(${target_name} PRIVATE unofficial::bgfx::bgfx)
        foreach(bgfx_dependency unofficial::bgfx::bimg unofficial::bgfx::bx)
            if(TARGET ${bgfx_dependency})
                target_link_libraries(${target_name} PRIVATE ${bgfx_dependency})
            endif()
        endforeach()
        if(ANDROID)
            target_link_libraries(${target_name} PRIVATE android log vulkan EGL GLESv3)
        elseif(APPLE)
            find_library(METAL_LIBRARY Metal)
            find_library(QUARTZCORE_LIBRARY QuartzCore)
            target_link_libraries(${target_name} PRIVATE ${METAL_LIBRARY} ${QUARTZCORE_LIBRARY})
        elseif(UNIX)
            target_link_libraries(${target_name} PRIVATE dl pthread)
        endif()
        message(STATUS "KiriKiri bgfx: using vcpkg unofficial-bgfx package")
        return()
    endif()
    ...
    if(TARGET bgfx::bgfx)
        target_compile_definitions(${target_name} PRIVATE KIRIKIRI_HAS_BGFX=1)
        target_link_libraries(${target_name} PRIVATE bgfx::bgfx)
        foreach(bgfx_dependency bgfx::bimg bgfx::bx)
            if(TARGET ${bgfx_dependency})
                target_link_libraries(${target_name} PRIVATE ${bgfx_dependency})
            endif()
        endforeach()
        ...
        message(STATUS "KiriKiri bgfx: using vcpkg bgfx package")
        return()
    endif()

    if(NOT KIRIKIRI_BGFX_ROOT OR NOT KIRIKIRI_BIMG_ROOT OR NOT KIRIKIRI_BX_ROOT)
        message(FATAL_ERROR "KIRIKIRI_ENABLE_BGFX requires vcpkg bgfx/unofficial-bgfx or KIRIKIRI_BGFX_ROOT, KIRIKIRI_BIMG_ROOT and KIRIKIRI_BX_ROOT")
    endif()
