...
extern unzFile ZEXPORT unzOpen3 (const void* path, zlib_filefunc64_32_def* pzlib_filefunc64_32_def, int is64bitOpenFunction)
{
    return unzOpenInternal(path, pzlib_filefunc64_32_def, is64bitOpenFunction);
}
...
extern int ZEXPORT unzCheckCurrentFileCoherencyHeader(unzFile s, uInt* piSizeVar,
    ZPOS64_T* poffset_local_extrafield,
    uInt* psize_local_extrafield)
{
    return unz64local_CheckCurrentFileCoherencyHeader((unz64_s*)s, piSizeVar, poffset_local_extrafield, psize_local_extrafield);
}

extern ZPOS64_T ZEXPORT unzGetStartPos(unzFile s,
    uInt piSizeVar)
{
    return ((unz64_s*)s)->cur_file_info_internal.offset_curfile + SIZEZIPLOCALHEADER + piSizeVar;
}
