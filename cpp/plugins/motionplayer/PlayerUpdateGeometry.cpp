                    ...
                    if(vn.meshType == 1 && !vn.meshControlPoints.empty() &&
                       cw > 0 && ch > 0) {
                        ...
                        if(std::fabs(det) > 1e-10) {
                            ...
                            // node+2128/2132: negated origin as float
                            // (0x6BCC04/0x6BCC38)
                        ...
                        if(divTotal < 1)
                            divTotal = 4;
                        const int divX =
                            static_cast<int>(static_cast<double>(divTotal) *
                                             cw / (cw + ch)) +
                            1;
                        ...
                        // NEON version at 0x6BB030..0x6BB138 processes 4
                        // points/iteration. Each row interpolates linearly
                        // between two edge points:
                        //   p0 = orgXY + m_col2*ch*tv, p1 = orgXY + m_col1*cw +
                        //   m_col2*ch*tv grid[gx] = lerp(p0, p1, gx/divX)
                        vn.meshControlPoints.resize(numPts * 2);
                        for(int gy = 0; gy < divY; ++gy) {
                            const double tv = (divY > 1)
                            ...
                            float *rowPtr =
                                &vn.meshControlPoints[gy * divX * 2];
#ifdef __EMSCRIPTEN__
                            // WASM SIMD: process 4 grid points per iteration
                            // Aligned to NEON at 0x6BB0CC..0x6BB138
                            // For each group of 4 gx values: tu = [gx, gx+1,
                            // gx+2, gx+3] / divX ptX = rowBaseX*(1-tu) +
                            // rowEndX*tu ptY = rowBaseY*(1-tu) + rowEndY*tu
                            ...
                            const double invDivX =
                                (divX > 1) ? 1.0 / (divX - 1) : 0.0;
                            int gx = 0;
                            const int simdEnd =
                                divX & ~1; // process 2 at a time (f64x2)
                            for(; gx < simdEnd; gx += 2) {
                                const double t0 = gx * invDivX;
                                const double t1 = (gx + 1) * invDivX;
                                const v128_t vt = wasm_f64x2_make(t0, t1);
                                const v128_t v1mt =
                                    wasm_f64x2_sub(wasm_f64x2_splat(1.0), vt);
                                // X = base*(1-t) + end*t
                                v128_t vx =
                                    wasm_f64x2_add(wasm_f64x2_mul(vBaseX, v1mt),
                                                   wasm_f64x2_mul(vEndX, vt));
                                // Y = base*(1-t) + end*t
                                v128_t vy =
                                    wasm_f64x2_add(wasm_f64x2_mul(vBaseY, v1mt),
                                                   wasm_f64x2_mul(vEndY, vt));
                                // Convert f64→f32 and store interleaved
                                // [x0,y0,x1,y1]
                                float fx0 = static_cast<float>(
                                    wasm_f64x2_extract_lane(vx, 0));
                                float fy0 = static_cast<float>(
                                    wasm_f64x2_extract_lane(vy, 0));
                                float fx1 = static_cast<float>(
                                    wasm_f64x2_extract_lane(vx, 1));
                                float fy1 = static_cast<float>(
                                    wasm_f64x2_extract_lane(vy, 1));
                                rowPtr[gx * 2] = fx0;
                                rowPtr[gx * 2 + 1] = fy0;
                                rowPtr[gx * 2 + 2] = fx1;
                                rowPtr[gx * 2 + 3] = fy1;
                            ...
                            // Scalar remainder
                            for(; gx < divX; ++gx) {
                                const double tu = (divX > 1)
                                    ? static_cast<double>(gx) / (divX - 1)
                                    : 0;
                                rowPtr[gx * 2] = static_cast<float>(
                                    rowBaseX * (1 - tu) + rowEndX * tu);
                                rowPtr[gx * 2 + 1] = static_cast<float>(
                        ...
                        // Evaluate each grid point through Bezier patch
                        // (0x6BCF80..0x6BCFBC) sub_69B1E8 evaluates bezier
                        // patch at each mesh point This transforms the bilinear
                        // grid into a deformed mesh
                        if(vn.meshControlPointsPrev.size() >= 32) {
                            auto evalBP = [](const float *mesh, float u,
                                             float v, float &outX,
                                             float &outY) {
                            ...
                            for(int pi = 0; pi < numPts; ++pi) {
                                ...
                                evalBP(vn.meshControlPointsPrev.data(), px, py,
                                       px, py);
                        ...
                        // Walk node+1968 (parentClipIndex), for each
                        // mesh-enabled ancestor: evaluate all mesh points +
                        // origin through its mesh Parent clip chain mesh
                        // cascade (0x6BD118..0x6BD380)
                        auto evalBPCascade = [](const float *mesh, float u,
                                                float v, float &outX,
                                                float &outY) {
                            const float su = 1.f - u, sv = 1.f - v;
                            const float bu[4] = { su * su * su,
                        ...
                        while(clipWalk >= 0 &&
                            ...
                            if(cn.meshControlPoints.size() >= 32) {
                                const float *cmesh =
                                    cn.meshControlPoints.data();
                                // Evaluate each mesh point through parent mesh
                                // (0x6BD148..0x6BD1E8)
                                for(size_t mi = 0;
                                    ...
                                    // Transform by parent inverse matrix +
                                    // offset (0x6BD188) Transform by parent
                                    // inverse matrix + offset (0x6BD188)
                                    float tx =
                                        mpx + cn.meshInvOffX; // node+2128
                                    float ty =
                                        mpy + cn.meshInvOffY; // node+2132
                                    // Apply inverse matrix:
                                    // [invM11,invM12;invM21,invM22] × (tx,ty)
                                    float ix =
                                        static_cast<float>(cn.meshInvM11 * tx +
                                                           cn.meshInvM12 * ty);
                                    float iy =
                                        static_cast<float>(cn.meshInvM21 * tx +
                                                           cn.meshInvM22 * ty);
                                    tx = ix;
                                    ty = iy;
                                    // Evaluate through parent bezier
                                    // (sub_69B1E8)
                                    float rx, ry;
                                ...
                                // Evaluate origin through parent mesh
                                // (0x6BD218..0x6BD258)
                                float cox = static_cast<float>(cascadeOrgY) +
                                    cn.meshInvOffY;
                                float coy = static_cast<float>(cascadeOrgX) +
                                    cn.meshInvOffX;
                                float rox, roy;
...
#ifdef __EMSCRIPTEN__
                            // WASM SIMD: process 4 floats at a time (2 XY
                            // pairs) Aligned to NEON at 0x6BD360: vadd with
                            // delta vector
                            const v128_t vdelta =
                                wasm_f32x4_make(fdx, fdy, fdx, fdy);
                            size_t fi = 0;
