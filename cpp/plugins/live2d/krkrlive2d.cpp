...
// Uses libarchive to read directly from memory (no temp file).
// ---------------------------------------------------------------------------
...
static bool ExtractZipToMemory(const uint8_t *zipData, size_t zipSize,
    ...
    if (archive_read_open_memory(arc, zipData, zipSize) != ARCHIVE_OK) {
    ...
    while (archive_read_next_header(arc, &entry) == ARCHIVE_OK) {
        ...
        bool ok = true;
        for (;;) {
            la_ssize_t read = archive_read_data(arc, chunk.data(), chunk.size());
            if (read < 0) {
                ok = false;
                break;
            }
        ...
        if (ok) out[std::string(name)] = std::move(data);
    }
...
public:
    ...
    bool LoadFromL2D(const std::vector<uint8_t> &zipData, const std::string &baseName) {
        ...
        csmUint32 rendererW = static_cast<csmUint32>(GetModel()->GetCanvasWidthPixel());
        csmUint32 rendererH = static_cast<csmUint32>(GetModel()->GetCanvasHeightPixel());
        if (rendererW == 0) rendererW = static_cast<csmUint32>(std::max(1, renderWidth_));
        if (rendererH == 0) rendererH = static_cast<csmUint32>(std::max(1, renderHeight_));
        CreateRenderer(rendererW, rendererH);
