...
BOOL CQR_Encode::EncodeData(int nLevel, int nVersion, BOOL bAutoExtent, int nMaskingNo, LPCSTR lpsSource, int ncSource)
{
	...
	int ncLength = ncSource > 0 ? ncSource : static_cast<int>(std::strlen(lpsSource));
	...
	std::memset(m_byAllCodeWord, 0, m_ncAllCodeWord);
	...
	for (i = 0; i < ncBlock1; ++i)
	{
		std::memset(m_byRSWork, 0, sizeof(m_byRSWork));

		std::memmove(m_byRSWork, m_byDataCodeWord + nDataCwIndex, ncDataCw1);
	...
	for (i = 0; i < ncBlock2; ++i)
	{
		...
		std::memmove(m_byRSWork, m_byDataCodeWord + nDataCwIndex, ncDataCw2);
...
{
	std::memset(m_nBlockLength, 0, sizeof(m_nBlockLength));
	...
	std::memset(m_byDataCodeWord, 0, MAX_DATACODEWORD);
...
void CQR_Encode::FormatModule()
{
	...
	std::memset(m_byModuleData, 0, sizeof(m_byModuleData));
